...
+ [Что такое JPA?](jpa.md#Что-такое-JPA)
+ [Из чего состоит JPA?](jpa.md#Из-чего-состоит-JPA)
+ [В чем её отличие JPA от Hibernate?](jpa.md#В-чем-её-отличие-JPA-от-Hibernate)
+ [В чем её отличие JPA от JDO?](jpa.md#В-чем-её-отличие-JPA-от-JDO)
+ [Можно ли использовать JPA c noSQL базами?](jpa.md#Можно-ли-использовать-JPA-c-noSQL-базами)
+ [Что такое JPQL (Java Persistence query language) и чем он отличается от SQL?](jpa.md#Что-такое-JPQL-Java-Persistence-query-language-и-чем-он-отличается-от-SQL)
+ [Что означает полиморфизм (polymorphism) в запросах JPQL (Java Persistence query language) и как его "выключить"?](jpa.md#Что-означает-полиморфизм-polymorphism-в-запросах-JPQL-Java-Persistence-query-language-и-как-его-выключить)
+ [Что такое Criteria API и для чего он используется?](jpa.md#Что-такое-Criteria-API-и-для-чего-он-используется)
+ [Что такое Entity?](jpa.md#Что-такое-Entity)
+ [Может ли не Entity класс наследоваться от Entity класса?](jpa.md#Может-ли-не-Entity-класс-наследоваться-от-Entity-класса)
+ [Может ли Entity класс наследоваться от других Entity классов?](jpa.md#Может-ли-Entity-класс-наследоваться-от-других-Entity-классов)
+ [Может ли Entity быть абстрактным классом?](jpa.md#Может-ли-Entity-быть-абстрактным-классом)
+ [Может ли Entity класс наследоваться от не Entity классов (non-entity classes)?](jpa.md#Может-ли-Entity-класс-наследоваться-от-не-Entity-классов-non-entity-classes)
+ [Какие требования JPA к Entity классам вы можете перечислить (не менее шести требований)?](jpa.md#Какие-требования-JPA-к-Entity-классам-вы-можете-перечислить-не-менее-шести-требований)
+ [Что такое атрибут Entity класса в терминологии JPA?](jpa.md#Что-такое-атрибут-Entity-класса-в-терминологии-JPA)
+ [Какие два типа элементов есть у Entity классов. Или другими словами перечислите два типа доступа (access) к элементам Entity классов.](jpa.md#Какие-два-типа-элементов-есть-у-Entity-классов-Или-другими-словами-перечислите-два-типа-доступа-access-к-элементам-Entity-классов)
+ [Какие типы данных допустимы в атрибутах Entity класса (полях или свойствах)?](jpa.md#Какие-типы-данных-допустимы-в-атрибутах-Entity-класса-полях-или-свойствах)
+ [Какие типы данных можно использовать в атрибутах, входящих в первичный ключ Entity класса (составной или простой), чтобы полученный первичный ключ мог использоваться для любой базы данных? А в случае автогенерируемого первичного ключа (generated primary keys)?](jpa.md#Какие-типы-данных-можно-использовать-в-атрибутах-входящих-в-первичный-ключ-Entity-класса-составной-или-простой-чтобы-полученный-первичный-ключ-мог-использоваться-для-любой-базы-данных-А-в-случае-автогенерируемого-первичного-ключа-generated-primary-keys)
+ [Что такое встраиваемый (Embeddable) класс?](jpa.md#Что-такое-встраиваемый-Embeddable-класс)
+ [Может ли встраиваемый (Embeddable) класс содержать другой встраиваемый (Embeddable) класс?](jpa.md#Может-ли-встраиваемый-Embeddable-класс-содержать-другой-встраиваемый-Embeddable-класс)
+ [Может ли встраиваемый (Embeddable) класс содержать связи (relationship) с другими Entity или коллекциями Entity? Если может, то существуют ли какие-то ограничение на такие связи (relationship)?](jpa.md#Может-ли-встраиваемый-Embeddable-класс-содержать-связи-relationship-с-другими-Entity-или-коллекциями-Entity-Если-может-то-существуют-ли-какие-то-ограничение-на-такие-связи-relationship)
+ [Какие требования JPA устанавливает к встраиваемым (Embeddable) классам?](jpa.md#Какие-требования-JPA-устанавливает-к-встраиваемым-Embeddable-классам)
+ [Какие типы связей (relationship) между Entity вы знаете (перечислите восемь типов, либо укажите четыре типа связей, каждую из которых можно разделить ещё на два вида)?](jpa.md#Какие-типы-связей-relationship-между-Entity-вы-знаете-перечислите-восемь-типов-либо-укажите-четыре-типа-связей-каждую-из-которых-можно-разделить-ещё-на-два-вида)
+ [Что такое Mapped Superclass?](jpa.md#Что-такое-Mapped-Superclass)
+ [Какие два типа fetch стратегии в JPA вы знаете?](jpa.md#Какие-два-типа-fetch-стратегии-в-JPA-вы-знаете)
+ [Какие три типы стратегии наследования мапинга (Inheritance Mapping Strategies) описаны в JPA?](jpa.md#Какие-три-типы-стратегии-наследования-мапинга-Inheritance-Mapping-Strategies-описаны-в-JPA)

+ [Что такое EntityManager и какие основные его функции вы можете перечислить?](jpa.md#Что-такое-EntityManager-и-какие-основные-его-функции-вы-можете-перечислить)
+ [Какие четыре статуса жизненного цикла Entity объекта (Entity Instance’s Life Cycle) вы можете перечислить?](jpa.md#Какие-четыре-статуса-жизненного-цикла-Entity-объекта-Entity-Instance’s-Life-Cycle-вы-можете-перечислить)
+ [Как влияет операция merge на Entity объекты каждого из четырех статусов?](jpa.md#Как-влияет-операция-merge-на-Entity-объекты-каждого-из-четырех-статусов)
+ [Как влияет операция remove на Entity объекты каждого из четырех статусов?](jpa.md#Как-влияет-операция-remove-на-Entity-объекты-каждого-из-четырех-статусов)
+ [Как влияет операция persist на Entity объекты каждого из четырех статусов?](jpa.md#Как-влияет-операция-persist-на-Entity-объекты-каждого-из-четырех-статусов)
+ [Как влияет операция refresh на Entity объекты каждого из четырех статусов?](jpa.md#Как-влияет-операция-refresh-на-Entity-объекты-каждого-из-четырех-статусов)
+ [Как влияет операция detach на Entity объекты каждого из четырех статусов?](jpa.md#Как-влияет-операция-detach-на-Entity-объекты-каждого-из-четырех-статусов)
+ [Для чего нужна аннотация Access?](jpa.md#Для-чего-нужна-аннотация-Access)
+ [Для чего нужна аннотация Basic?](jpa.md#Для-чего-нужна-аннотация-Basic)
+ [Какой аннотациями можно перекрыть связи (override entity relationship) или атрибуты, унаследованные от суперкласса, или заданные в embeddable классе при использовании этого embeddable класса в одном из entity классов и не перекрывать в остальных?](jpa.md#Какой-аннотациями-можно-перекрыть-связи-override-entity-relationship-или-атрибуты-унаследованные-от-суперкласса-или-заданные-в-embeddable-классе-при-использовании-этого-embeddable-класса-в-одном-из-entity-классов-и-не-перекрывать-в-остальных)
+ [Какие аннотации служит для задания класса преобразования basic атрибута Entity в другой тип при сохранении/получении данных их базы (например, работать с атрибутом Entity boolean типа, но в базу сохранять его как число)?](jpa.md#Какие-аннотации-служит-для-задания-класса-преобразования-basic-атрибута-Entity-в-другой-тип-при-сохранении/получении-данных-их-базы-например-работать-с-атрибутом-Entity-boolean-типа-но-в-базу-сохранять-его-как-число)
+ [Какой аннотацией можно управлять кешированием JPA для данного Entity?](jpa.md#Какой-аннотацией-можно-управлять-кешированием-JPA-для-данного-Entity)
+ [Какой аннотацией можно задать класс, методы которого должен выполнится при определенных JPA операциях над данным Enitity или Mapped Superclass (такие как удаление, изменение данных и т.п.)?](jpa.md#Какой-аннотацией-можно-задать-класс-методы-которого-должен-выполнится-при-определенных-JPA-операциях-над-данным-Enitity-или-Mapped-Superclass-такие-как-удаление-изменение-данных-и-тп)
+ [Для чего нужны callback методы в JPA? К каким сущностям применяются аннотации callback методов? Перечислите семь callback методов (или что тоже самое аннотаций callback методов).](jpa.md#Для-чего-нужны-callback-методы-в-JPA-К-каким-сущностям-применяются-аннотации-callback-методов-Перечислите-семь-callback-методов-или-что-тоже-самое-аннотаций-callback-методов)
+ [Какой аннотацей можно исключить поли и свойства Entity из маппинга (property or field is not persistent)?](jpa.md#Какой-аннотацей-можно-исключить-поли-и-свойства-Entity-из-маппинга-property-or-field-is-not-persistent)
+ [Какие аннотации служить для установки порядка выдачи элементов коллекций Entity?](jpa.md#Какие-аннотации-служить-для-установки-порядка-выдачи-элементов-коллекций-Entity)
+ [Какие шесть видов блокировок (lock) описаны в спецификации JPA (или какие есть значения у enum LockModeType в JPA)?](jpa.md#Какие-шесть-видов-блокировок-lock-описаны-в-спецификации-JPA-или-какие-есть-значения-у-enum-LockModeType-в-JPA)
+ [Какие два вида кэшей (cache) вы знаете в JPA и для чего они нужны?](jpa.md#Какие-два-вида-кэшей-cache-вы-знаете-в-JPA-и-для-чего-они-нужны)
+ [Какие есть варианты настройки second-level cache (кэша второго уровня) в JPA или что аналогично опишите какие значения может принимать элемент shared-cache-mode из persistence.xml?](jpa.md#Какие-есть-варианты-настройки-second-level-cache-кэша-второго-уровня-в-JPA-или-что-аналогично-опишите-какие-значения-может-принимать-элемент-shared-cache-mode-из-persistencexml)
+ [Как можно изменить настройки fetch стратегии любых атрибутов Entity для отдельных запросов (query) или методов поиска (find), то если у Enity есть атрибут с fetchType = LAZY, но для конкретного запроса его требуется сделать EAGER или наоборот?](jpa.md#Как-можно-изменить-настройки-fetch-стратегии-любых-атрибутов-Entity-для-отдельных-запросов-query-или-методов-поиска-find-то-если-у-Enity-есть-атрибут-с-fetchType-=-LAZY-но-для-конкретного-запроса-его-требуется-сделать-EAGER-или-наоборот)
+ [Каким способом можно получить метаданные JPA (сведения о Entity типах, Embeddable и Managed классах и т.п.)?](jpa.md#Каким-способом-можно-получить-метаданные-JPA-сведения-о-Entity-типах-Embeddable-и-Managed-классах-и-тп)
+ [Каким способом можно в коде работать с кэшем второго уровня (удалять все или определенные Entity из кеша, узнать закэшировался ли данное Entity и т.п.)?](jpa.md#Каким-способом-можно-в-коде-работать-с-кэшем-второго-уровня-удалять-все-или-определенные-Entity-из-кеша-узнать-закэшировался-ли-данное-Entity-и-тп)
+ [В чем разница в требованиях к Entity в Hibernate, от требований к Entity, указанных в спецификации JPA?](jpa.md#В-чем-разница-в-требованиях-к-Entity-в-Hibernate-от-требований-к-Entity-указанных-в-спецификации-JPA)
+ [Какая уникальная стратегия наследования есть в Hibernate, но нет в спецификации JPA?](jpa.md#Какая-уникальная-стратегия-наследования-есть-в-Hibernate-но-нет-в-спецификации-JPA)
+ [Какие основные новые возможности появились в спецификации JPA 2.1 по сравнению с JPA 2.0 (перечислите хотя бы пять-шесть новых возможностей)?](jpa.md#Какие-основные-новые-возможности-появились-в-спецификации-JPA-21-по-сравнению-с-JPA-20-перечислите-хотя-бы-пять-шесть-новых-возможностей)
...
+ [Какие преимущства от использования Hibernate?](hibernate.md#Какие-преимущства-от-использования-Hibernate)
+ [Как Hibernate помогает в программировании?](hibernate.md#Как-Hibernate-помогает-в-программировании)
+ [Какие преимущества Hibernate над JDBC?](hibernate.md#Какие-преимущества-Hibernate-над-JDBC)
+ [Что такое конфигурационный файл Hibernate?](hibernate.md#Что-такое-конфигурационный-файл-Hibernate)
+ [Способы конфигурации работы с Hibernate.](hibernate.md#Способы-конфигурации-работы-с-Hibernate)
+ [Что такое Hibernate mapping file?](hibernate.md#Что-такое-Hibernate-mapping-file)
+ [Что такое Переходные объекты (Transient Objects)?](hibernate.md#Что-такое-Переходные-объекты-Transient-Objects)
+ [Что такое постоянные объекты (Persistent objects)?](hibernate.md#Что-такое-постоянные-объекты-Persistent-objects)
+ [Что такое TransactionFactory?](hibernate.md#Что-такое-TransactionFactory)
+ [Что такое ConnectionProvider?](hibernate.md#Что-такое-ConnectionProvider)
+ [Что такое Трансакция (Transaction)?](hibernate.md#Что-такое-Трансакция-Transaction)
+ [Какие существуют стратегии загрузки объектов в Hibernate?](hibernate.md#Какие-существуют-стратегии-загрузки-объектов-в-Hibernate)
+ [Какие бывают id generator классы в Hibernate?](hibernate.md#Какие-бывают-id-generator-классы-в-Hibernate)
+ [Какие ключевые интерфейсы использует Hibernate?](hibernate.md#Какие-ключевые-интерфейсы-использует-Hibernate)
+ [Назовите некоторые важные аннотации, используемые для отображения в Hibernate.](hibernate.md#Назовите-некоторые-важные-аннотации-используемые-для-отображения-в-Hibernate)
+ [Какая роль интерфейса Session в Hibernate?](hibernate.md#Какая-роль-интерфейса-Session-в-Hibernate)
+ [Какая роль интерфейса SessionFactory в Hibernate?](hibernate.md#Какая-роль-интерфейса-SessionFactory-в-Hibernate)
+ [Является ли Hibernate SessionFactory потокобезоспансым?](hibernate.md#Является-ли-Hibernate-SessionFactory-потокобезоспансым)
+ [В чем разница между openSession и getCurrentSession?](hibernate.md#В-чем-разница-между-openSession-и-getCurrentSession)
+ [Какие типы коллекций представлены в Hibernate?](hibernate.md#Какие-типы-коллекций-представлены-в-Hibernate)
+ [Какие типы менеджмента транзакций поддерживаются в Hibernate?](hibernate.md#Какие-типы-менеджмента-транзакций-поддерживаются-в-Hibernate)
+ [Что собой являет коллекция типа Bag и зачем она используется?](hibernate.md#Что-собой-являет-коллекция-типа-Bag-и-зачем-она-используется)
+ [Какие типы кэша используются в Hibernate?](hibernate.md#Какие-типы-кэша-используются-в-Hibernate)
+ [Какие существуют типы стратегий кэша?](hibernate.md#Какие-существуют-типы-стратегий-кэша)
+ [Что вы знаете о кэширование в Hibernate? Объясните понятие кэш первого уровня в Hibernate?](hibernate.md#Что-вы-знаете-о-кэширование-в-Hibernate-Объясните-понятие-кэш-первого-уровня-в-Hibernate)
+ [Как настраивается кэш второго уровня в Hibernate?](hibernate.md#Как-настраивается-кэш-второго-уровня-в-Hibernate)
+ [Какая разница в работе методов load(); и get();?](hibernate.md#Какая-разница-в-работе-методов-load-и-get)
+ [Каковы существуют различные состояния у entity bean?](hibernate.md#Каковы-существуют-различные-состояния-у-entity-bean)
+ [Что произойдет, если будет отсутствовать конструктор без аргументов у Entity Bean?](hibernate.md#Что-произойдет-если-будет-отсутствовать-конструктор-без-аргументов-у-Entity-Bean)
+ [Как используется вызов метода Hibernate Session merge()?](hibernate.md#Как-используется-вызов-метода-Hibernate-Session-merge)
+ [В чем разница между Hibernate save(), saveOrUpdate() и persist()?](hibernate.md#В-чем-разница-между-Hibernate-save-saveOrUpdate-и-persist)
+ [Что такое Lazy fetching(изъятие) в Hibernate?](#Что-такое-Lazy-fetching-изъятие-в-Hibernate)
+ [В чем разница между sorted collection и ordered collection? Какая из них лучше?](hibernate.md#В-чем-разница-между-sorted-collection-и-ordered-collection-Какая-из-них-лучше)
+ [Как реализованы Join’ы Hibernate?](hibernate.md#Как-реализованы-Join-ы-Hibernate)
+ [Почему мы не должны делать Entity class как final?](hibernate.md#Почему-мы-не-должны-делать-Entity-class-как-final)
+ [Что вы знаете о HQL и каковы его преимущества?](hibernate.md#Что-вы-знаете-о-HQL-и-каковы-его-преимущества?)
+ [Что такое Query Cache в Hibernate?](hibernate.md#Что-такое-Query-Cache-в-Hibernate)
+ [Можем ли мы выполнить SQL (sql native) запрос в Hibernate?](hibernate.md#Можем-ли-мы-выполнить-SQL-sql-native-запрос-в-Hibernate)
+ [Назовите преимущества поддержки нативного sql в Hibernate.](hibernate.md#Назовите-преимущества-поддержки-нативного-sql-в-Hibernate)
+ [Что такое Named SQL Query?](hibernate.md#Что-такое-Named-SQL-Query)
+ [Каковы преимущества Named SQL Query?](hibernate.md#Каковы-преимущества-Named-SQL-Query)
+ [Как добавить логирование log4j в Hibernate приложение?](hibernate.md#Как-добавить-логирование-log4j-в-Hibernate-приложение)
+ [Как логировать созданные Hibernate SQL запросы в лог-файлы?](hibernate.md#Как-логировать-созданные-Hibernate-SQL-запросы-в-лог-файлы)
+ [Что вы знаете о Hibernate прокси и как это помогает в ленивой загрузке (lazy load)?](hibernate.md#Что-вы-знаете-о-Hibernate-прокси-и-как-это-помогает-в-ленивой-загрузке-lazy-load)
+ [Как управлять транзакциями с помощью Hibernate?](hibernate.md#Как-управлять-транзакциями-с-помощью-Hibernate)
+ [Что такое каскадные связи (обновления) в Hibernate?](hibernate.md#Что-такое-каскадные-связи-обновления-в-Hibernate)
+ [Какие каскадные типы есть в Hibernate?](hibernate.md#Какие-каскадные-типы-есть-в-Hibernate)
+ [Что такое сесиия и фаблика сессий в Hibernate? Как настроить session factory в конфигурационном файле Spring?](#hibernate.mdЧто-такое-сесиия-и-фаблика-сессий-в-Hibernate-Как-настроить-session-factory-в-конфигурационном-файле-Spring)
+ [Как использовать JNDI DataSource сервера приложений с Hibernate Framework?](hibernate.md#Как-использовать-JNDI-DataSource-сервера-приложений-с-Hibernate-Framework)
+ [Как интегрировать Hibernate и Spring?](hibernate.md#Как-интегрировать-Hibernate-и-Spring)
+ [Какие паттерны применяются в Hibernate?](hibernate.md#Какие-паттерны-применяются-в-Hibernate)
+ [Расскажите о Hibernate Validator Framework.](hibernate.md#Расскажите-о-Hibernate-Validator-Framework.)
+ [Какие преимущества дает использование плагина Hibernate Tools Eclipse?](hibernate.md#Какие-преимущества-дает-использование-плагина-Hibernate-Tools-Eclipse)
